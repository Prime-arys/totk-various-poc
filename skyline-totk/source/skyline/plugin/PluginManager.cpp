...
namespace {
    // Callbacks plugins register from their `main` to run once every plugin's
    // `main` has returned. Plugins run in whatever order the filesystem lists
    // them, so anything that depends on another plugin (e.g. the mod merger
    // waiting for a plugin that picks which mods to load) hooks in here instead.
    struct LoadedCallback {
        void (*callback)(void*);
        void* user;
    };

    constexpr int MaxLoadedCallbacks = 32;
    LoadedCallback g_LoadedCallbacks[MaxLoadedCallbacks];
    int g_LoadedCallbackCount = 0;
...
namespace plugin {
    ...
    void Manager::LoadPluginsImpl() {
        ...
        // Collect plugins from the SD card first, then from romfs:/skyline/plugins
        // for compatibility with the usual Skyline layout. The same NRO found in
        // both places is only loaded once (see the hash check below).
        auto collect = [this](nn::fs::DirectoryEntry const& entry, std::shared_ptr<std::string> path) {
            if (entry.type == nn::fs::DirectoryEntryType_File)  // ignore directories
                m_pluginInfos.push_back(PluginInfo{.Path = *path});
        };

        if (!totk::g_Config.plugins_dir.empty()) skyline::utils::walkDirectory(totk::g_Config.plugins_dir, collect);

        size_t fromSd = m_pluginInfos.size();
        skyline::utils::walkDirectory(utils::g_RomMountStr + PLUGIN_PATH, collect);

        if (m_pluginInfos.size() > fromSd) {
            skyline::logger::s_Instance->LogFormat(
                "[PluginManager] %zu plugin(s) come from romfs. If nothing else lives in the title's romfs "
                "folder, moving them to %s stops Atmosphere from building a layered romfs at every boot.",
                m_pluginInfos.size() - fromSd, totk::g_Config.plugins_dir.c_str());
        }
        ...
        *nrrHeader = nn::ro::NrrHeader{
        ...
        for (int i = 0; i < g_LoadedCallbackCount; i++) {
            skyline::logger::s_Instance->LogFormat("[PluginManager] Running plugins-loaded callback %d", i);
            g_LoadedCallbacks[i].callback(g_LoadedCallbacks[i].user);
        }
    }
...
// Runs `callback(user)` after every plugin's `main` has returned, still before
// the game resumes loading. Returns false if the table is full, or if plugins
// have already finished loading (the callback then runs immediately).
bool skyline_totk_on_plugins_loaded(void (*callback)(void*), void* user) {
    ...
    if (g_LoadedCallbackCount >= MaxLoadedCallbacks) return false;

    g_LoadedCallbacks[g_LoadedCallbackCount++] = {callback, user};
    return true;
}
