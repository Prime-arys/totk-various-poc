...
void Logger::StartThread() {
    ...
    if (R_FAILED(nn::os::CreateThread(thread, ThreadMain, this, threadStack, stackSize, 16))) return;
...
void Logger::Log(const char* data, size_t size) {
    ...
    // No flushing thread yet: write it out now rather than allocating a queue.
    if (!m_threaded) {
        SendRaw((void*)data, size);
        return;
    }
...
void Logger::LogFormat(const char* format, ...) {
    ...
    if (!m_threaded) {
        char buff[0x400];
        int len = vsnprintf(buff, sizeof(buff) - 2, format, args);
        va_end(args);
        if (len < 0) return;
        if (len > (int)sizeof(buff) - 2) len = sizeof(buff) - 2;
        buff[len] = '\n';
        buff[len + 1] = '\0';
        SendRaw(buff, len + 1);
        return;
