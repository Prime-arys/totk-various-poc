...
bool SafeTaskQueue::startThread(s32 priority, s32 core, u64 stackSize) {
    skyline::logger::s_Instance->Log("[SafeTaskQueue] Starting thread.\n");
    void* stack = memalign(0x1000, stackSize);

    // A negative core means "whatever the process defaults to", which is the
    // only safe choice unless the game's NPDM is known to allow that core.
    Result rc = core < 0 ? nn::os::CreateThread(&thread, entrypoint, this, stack, stackSize, priority)
